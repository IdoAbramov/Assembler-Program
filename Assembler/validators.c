...
int isEmptyOrCommentLine(const char* line)
{
	...
	while(line[i] != '\n' && line[i] != '\0') /* loop till it's next line or end of string */
	{
		...
		if(line[i] != '\t' && line[i] != ' ') /* if there are chars which are not tabs or spaces - means its not an empty line */
		{
			return FALSE;
		}
...
int isLegalNumber(const char* strNum)
{
	...
	if(num > MAX_NUMBER_SIZE || num < MIN_NUMBER_SIZE) /*checks if the number is in the range of 12 bits signed numebr */
	{
		return FALSE;	
	}
