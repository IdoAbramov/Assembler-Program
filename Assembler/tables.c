...
void addRegisterWordIntoCodeTable(char* reg)
{
	...
	while(i < NUM_OF_REGISTERS && strcmp(registers[i],reg) != 0)
	{
		i++;
	}
...
void addDirectAddressWordIntoCodeTable(char* label)
{
	...
	while(currentLabelPtr != NULL && strcmp(currentLabelPtr->name,label) != 0) /*checks if the label exists in the Labels Table*/
	{
...
void addRelativeAddressWordIntoCodeTable(char* label)/*adding an address word of relative label (label's address - IC)*/
{
	...
	while(currentLabelPtr != NULL && strcmp(currentLabelPtr->name,(label+1)) != 0)/*checks if the label exists in the Labels Table (points to the second char beacuse the first one is '%')*/
	{
		currentLabelPtr = currentLabelPtr->next;
	}	
...
void updateDataTableAddresses()
{
	...
	{
		if(currentLabelPtr->type == DATA || currentLabelPtr->type == DATA_ENTRY)
		{
...
int checkForEntry()
{
	...
	else /*if there are labels inside the labels table*/
	{
		...
		{	
			if(currentLabelPtr->type == DATA_ENTRY || currentLabelPtr->type == CODE_ENTRY)
			{
				return TRUE;
			}
			currentLabelPtr = currentLabelPtr->next;
...
void printAllErrors()
{
	...
	{	
		printf("\n- File:\"%s\" line:%d: %s", currentErrorPtr->file,currentErrorPtr->line, currentErrorPtr->description);
		currentErrorPtr = currentErrorPtr->next;
	}
}
