...
void firstRound(FILE* f1, char* fileName)
{
	...
	while(fgets(lineBuffer, LINE_MAX_LENGTH, f1) != NULL) { /*reads from the input file line by line until the end of file*/
		...
		if(word == NULL ||
		   label == NULL) { /*if the allocation of memory failed*/ 
			memoryAllocationFailure();/*termiantes the program*/
		}
...
void twoOperandsOpcode(char* opcode, char* src, char* dest, char* fileName)
{
	...
	while((strcmp(opcodeAddressModes[i].opcodeName, opcode) != 0) &&
	      (i < NUM_OF_OPCODES)) {/*gets the opcode index in the addressing modes array*/
		i++;
	}
	...
	if((srcAddressMode != NO_ADDRESS_MODE) &&
	   (destAddressMode != NO_ADDRESS_MODE)) {/*if both source and destination operands has valid address modes*/
		...
		addOpcodeWordIntoCodeTable(opcodeAddressModes[i].opcode, 
					   opcodeAddressModes[i].funct, 
					   destAddressMode, 
					   srcAddressMode);
		...
		if (line[i] == ',') { /*checks if the char is a comma*/
			...
			if(line[i] != '\0') { /*if there are more chars*/
				if(line[i] == ',') { /*check if char is comma*/
					...
					while(line[i] != ',' &&
					      line[i] != '\0' &&
					      isspace(line[i])) { /*stops where comma or end of string skipping whitespaces*/
						i++;
					}
				...
				if(commaFlag) { /*if there is comma, start to look for the destination operand*/
					while(line[i] != '\0' &&
					      line[i] != ',' &&
					      isspace(line[i])) {/*skipping the whitespaces if exist*/
						i++;
					}
					...
					if(line[i] != '\0') { /*if no end of string, means there is destintation operand*/
						if (line[i] == ',') { /*if comma, too many commas - error*/
							...
							while(line[i] != '\0' &&
							      !(isspace(line[i])) &&
							      line[i] != ',') { /*get the destination operand*/
								dest[j] = line[i];
								i++;
								j++;
							}
							...
							while(line[i] !='\0' &&
							      isspace(line[i])) {/*skips the next whitespaces*/
								i++;
							}
...
{	
	int i=0;
	while((strcmp(opcodeAddressModes[i].opcodeName, opcode) != 0) &&
	      (i < NUM_OF_OPCODES)) { /*gets the index of the opcode by its name from the opcode address modes array */
		i++;
	}

	if(isOperandNumber(src) &&
	   (opcodeAddressModes[i].srcMode.immediate)) { /*checks if the source operand is a valid number and if the
							  source operand allows "immediate" address mode */
		return IMMEDIATE;/*if so, returns the addressing mode - immediate addressing*/
	}

	if(isLabelAlreadyInTable(src) &&
	   (opcodeAddressModes[i].srcMode.direct)) {  /*checks if the source operand is a valid label and if
							source operand allows "direct" address mode*/
		return DIRECT;/*if so, returns the addressing mode - direct addressing*/
	}

	if(isRelativeAddress(src) &&
	   (opcodeAddressModes[i].srcMode.relative)) { /*checks if the source operand is a valid relative label and if
							 source operand allows "relative" address mode*/
		return RELATIVE; /*if so, returns the addressing mode - relative addressing*/
	}

	if(isRegister(src) &&
	   (opcodeAddressModes[i].srcMode.direct_register)) { /*checks if the source operand is a valid register and if
...
{	
	int i=0;
	while((strcmp(opcodeAddressModes[i].opcodeName, opcode) != 0) &&
	      (i < NUM_OF_OPCODES)) { /*gets the index of the opcode by its name 
					from the opcode address modes array */
		i++;
	}

	if(isOperandNumber(dest) &&
	   (opcodeAddressModes[i].destMode.immediate)) { /*checks if the destination operand is a valid number and if the
							   destination operand allows "immediate" address mode */
		return IMMEDIATE;/*if so, returns the addressing mode - immediate addressing*/
	}

	if(isLabelAlreadyInTable(dest) &&
	   (opcodeAddressModes[i].destMode.direct)) { /*checks if the destination operand is a valid label and if
							destination operand allows "direct" address mode*/
		return DIRECT;/*if so, returns the addressing mode - direct addressing*/
	}

	if(isRelativeAddress(dest) &&
	   (opcodeAddressModes[i].destMode.relative)) { /*checks if the destination operand is a valid relative label and if
							  destination operand allows "relative" address mode*/
		return RELATIVE;/*if so, returns the addressing mode - relative addressing*/
	}

	if(isRegister(dest) && 
	   (opcodeAddressModes[i].destMode.direct_register)) { /*checks if the destination operand is a valid register and if
